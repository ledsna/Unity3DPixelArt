...
#pragma warning restore CS0618
        ...
        internal class PassData
        {
            ...
            internal TextureHandle pixelPerfectDetailHdl;
        ...
        internal void Render(RenderGraph renderGraph, ContextContainer frameData, TextureHandle colorTarget, TextureHandle depthTarget, TextureHandle mainShadowsTexture, TextureHandle additionalShadowsTexture, uint batchLayerMask = uint.MaxValue, bool isMainOpaquePass = false, TextureHandle pixelPerfectDetailTexture = default)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<PassData>(passName, out var passData, profilingSampler))
            {
                ...
                if (pixelPerfectDetailTexture.IsValid())
                {
                    passData.pixelPerfectDetailHdl = pixelPerfectDetailTexture;
                    builder.SetRenderAttachment(pixelPerfectDetailTexture, 1, AccessFlags.Write);
                }
                ...
                {
                    bool hasPixelPerfectDetail = data.pixelPerfectDetailHdl.IsValid();
                    if (hasPixelPerfectDetail)
                        context.cmd.SetKeyword(ShaderGlobalKeywords.WritePixelPerfectDetail, true);
                    ...
                    if (hasPixelPerfectDetail)
                        context.cmd.SetKeyword(ShaderGlobalKeywords.WritePixelPerfectDetail, false);
                });
            }
        }
    }
    ...
    internal class DrawObjectsWithRenderingLayersPass : DrawObjectsPass
    {
        ...
        internal void Render(RenderGraph renderGraph, ContextContainer frameData, TextureHandle colorTarget, TextureHandle renderingLayersTexture, TextureHandle depthTarget, TextureHandle mainShadowsTexture, TextureHandle additionalShadowsTexture, RenderingLayerUtils.MaskSize maskSize, uint batchLayerMask = uint.MaxValue, TextureHandle pixelPerfectDetailTexture = default)
        {
            using (var builder = renderGraph.AddRasterRenderPass<RenderingLayersPassData>(passName, out var passData, profilingSampler))
            {
                ...
                {
                    passData.pixelPerfectDetailHdl = pixelPerfectDetailTexture;
                    passData.basePassData.pixelPerfectDetailHdl = pixelPerfectDetailTexture;
                    builder.SetRenderAttachment(pixelPerfectDetailTexture, 2, AccessFlags.Write);
                }
