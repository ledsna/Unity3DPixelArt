    ...
    public partial class DepthNormalOnlyPass : ScriptableRenderPass
    {
        ...
        internal void Render(RenderGraph renderGraph, ContextContainer frameData, in TextureHandle cameraNormalsTexture, in TextureHandle depthTexture, in TextureHandle renderingLayersTexture, uint batchLayerMask, bool setGlobalDepth, bool setGlobalNormalAndRenderingLayers, bool allowPartialPass)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<PassData>(passName, out var passData, profilingSampler))
            {
                ...
                if (resourceData.cameraObjectIDTexture.IsValid())
                {
                    int objectIDSlot = passData.enableRenderingLayers ? 2 : 1;
                    builder.SetRenderAttachment(resourceData.cameraObjectIDTexture, objectIDSlot, AccessFlags.Write);
                }
                ...
                if (setGlobalNormalAndRenderingLayers)
                {
                    ...
                    if (resourceData.cameraObjectIDTexture.IsValid())
                        builder.SetGlobalTextureAfterPass(resourceData.cameraObjectIDTexture, Shader.PropertyToID("_CameraObjectIDTexture"));
                }
                ...
                builder.SetRenderFunc((PassData data, RasterGraphContext context) =>
                {
                    ...
                    var writeObjectID = GlobalKeyword.Create("_WRITE_OBJECT_ID");
                    context.cmd.SetKeyword(writeObjectID, true);
                    
                    ExecutePass(context.cmd, data, data.rendererList);
                    
                    context.cmd.SetKeyword(writeObjectID, false);
                });
            }
        }
    }
