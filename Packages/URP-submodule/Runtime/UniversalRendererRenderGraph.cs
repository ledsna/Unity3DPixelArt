...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed partial class UniversalRenderer
    {
        ...
        void CreateRenderGraphCameraRenderTargets(RenderGraph renderGraph, bool isCameraTargetOffscreenDepth, bool requireIntermediateAttachments, bool depthTextureIsDepthFormat)
        {
            ...
            CreateCameraObjectIDTexture(renderGraph, cameraDescriptor);

            if (cameraData.renderScale > 1.0f)
                CreatePixelPerfectDetailTexture(renderGraph, cameraDescriptor);
                ...
                for (int passIndex = 0; passIndex < passCount; ++passIndex)
                {
                    ...
                    if (m_RenderingLayerProvidesRenderObjectPass)
                    {
                        m_RenderOpaqueForwardWithRenderingLayersPass.Render(
                            ...
                            batchLayerMask,
                            resourceData.pixelPerfectDetailTexture);
        ...
        void CreateCameraObjectIDTexture(RenderGraph renderGraph, TextureDesc descriptor)
        {
            UniversalResourceData resourceData = frameData.Get<UniversalResourceData>();
            descriptor.msaaSamples = MSAASamples.None; // Never use MSAA for the Object ID texture!
            descriptor.format = UnityEngine.Experimental.Rendering.GraphicsFormat.R32G32_SFloat; // Float2 format
            resourceData.cameraObjectIDTexture = CreateRenderGraphTexture(renderGraph, descriptor, "_CameraObjectIDTexture", true, Color.clear);
        }

        void CreatePixelPerfectDetailTexture(RenderGraph renderGraph, TextureDesc descriptor)
        {
            UniversalResourceData resourceData = frameData.Get<UniversalResourceData>();
            descriptor.format = UnityEngine.Experimental.Rendering.GraphicsFormat.R8_UNorm;
            resourceData.pixelPerfectDetailTexture = CreateRenderGraphTexture(renderGraph, descriptor, "_PixelPerfectDetailTexture", true, Color.black);
        }
